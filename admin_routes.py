...
function showAdminPanel() {
    ...
    html += '<canvas id="statsChart" width="400" height="200" style="max-width:100%"></canvas></div>';
...
function renderUserList(users) {
    ...
    html += '<th style="width:10%">手机号</th><th style="width:28%">API Key</th>';
    ...
    for (var i = 0; i < users.length; i++) {
        ...
        html += '<button class="delete-btn" onclick="del(\'' + escapedPhone + '\')">删除</button></td></tr>';
    }
    html += '</tbody></table>';
...
async function loadStats() {
    ...
    try {
        ...
        if (ctx) {
            ctx.innerHTML = '<div style="padding:20px;background:#f5f5f5;border-radius:8px;text-align:center">📊 每日调用次数: ' + (data.totalCalls || 0) + '次</div>';
        }
    } catch(e) { console.error(e); statsGrid.innerHTML = '<div>加载失败</div>'; }
}
